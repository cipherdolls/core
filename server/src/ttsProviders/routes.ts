...
const voiceInclude = { ttsVoices: { include: { _count: { select: { characters: true } } }, orderBy: [{ recommended: 'desc' as const }, { name: 'asc' as const }] }, _count: { select: { ttsVoices: true } } };
...
export const ttsProvidersRoutes = new Elysia({ prefix: '/tts-providers' })
  ...
  .get('/', async ({ user, query }) => {
    ...
    const [items, total] = await prisma.$transaction([
      prisma.ttsProvider.findMany({ skip, take, where, include: { picture: true, ...voiceInclude }, orderBy: { name: 'asc' } }),
  ...
  .get('/:id', async ({ params, set }) => {
    const item = await prisma.ttsProvider.findUnique({ where: { id: params.id }, include: { picture: true, ...voiceInclude } });
  ...
  .patch('/:id', async ({ user, params, body, set }) => {
    ...
    const updated = await model.ttsProvider.update({
      where: { id: params.id },
      data: pickFields(body, ['name', 'exampleVoiceText']),
      include: { _count: { select: { ttsVoices: true } } },
    }, original);
    return updated;
  }, {
