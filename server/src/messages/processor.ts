...
class MessagesProcessor extends BaseProcessor<Message> {
  ...
  protected override async onCreated(_job: Job, message: Message): Promise<void> {
    ...
    switch (message.role) {
      ...
      case 'SYSTEM':
        ...
        await this.createChatCompletion(message);
        break;
    }
  }
  ...
  private async handleUserCreated(message: Message): Promise<void> {
    ...
    if (hasAudio && hasText) {
      ...
      console.log(`[message] User text message. Enqueue chatCompletion.`);
      await this.createChatCompletion(message);
    } else {
  ...
  private async createChatCompletion(message: Message): Promise<void> {
