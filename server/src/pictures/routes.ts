...
export const picturesRoutes = new Elysia({ prefix: '/pictures' })
  ...
  .post('/', async ({ user, body, set }) => {
    ...
    const entityKeys = ['characterId', 'aiProviderId', 'sttProviderId', 'ttsProviderId'] as const;
    const provided = entityKeys.filter((k) => (body as any)[k]);
    if (provided.length !== 1) {
      set.status = 400;
      return { error: 'Provide exactly one entity ID (characterId, aiProviderId, sttProviderId, or ttsProviderId)' };
    }
