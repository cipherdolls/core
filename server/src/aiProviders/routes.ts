...
const include = { picture: true, chatModels: true, _count: { select: { chatModels: true } } };
...
export const aiProvidersRoutes = new Elysia({ prefix: '/ai-providers' })
  ...
  .get('/', async ({ user, query }) => {
    ...
    const [items, total] = await prisma.$transaction([
      prisma.aiProvider.findMany({ skip, take, where, include, orderBy: { name: 'asc' } }),
  ...
  .get('/:id', async ({ params, set }) => {
    const item = await prisma.aiProvider.findUnique({ where: { id: params.id }, include });
