...
export const aiProvidersRoutes = new Elysia({ prefix: '/ai-providers' })
  ...
  .get('/', async ({ user, query }) => {
    ...
    const [items, total] = await prisma.$transaction([
      prisma.aiProvider.findMany({ skip, take, where, include: { picture: true, chatModels: true, embeddingModels: true, reasoningModels: true, _count: { select: { chatModels: true, embeddingModels: true, reasoningModels: true } } }, orderBy: { name: 'asc' } }),
