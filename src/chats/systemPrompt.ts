...
export async function buildAndCacheSystemPrompt(chatId: string): Promise<string> {
  ...
  const char = chat.avatar.name;
  const user = chat.user.name ?? 'User';
  const userLanguage = chat.user.language ?? 'en';
  ...
  const promptText = md([
    ...
    // -- Avatar Personality --
    {
      ...
      body: isRoleplay
        ? [
            {
              text: `Name: ${char}\nGender: ${chat.avatar.gender ?? ''}\nPersonality:\n${chat.avatar.character ?? ''}`,
            },
          ]
        : [{ text: chat.avatar.character ?? '' }],
    },
    ...
    // -- Scenario --
    {
      ...
      body: [
        ...
        { text: `Current Scenario: ${chat.scenario.name}\nScenario Context:\n${chat.scenario.systemMessage ?? ''}` },
      ],
    },
    ...
    !isRoleplay && {
      ...
      body: [
        {
          text: `Current Time: ${new Date().toLocaleString()}\nLast time ${user} spoke to you: ${lastUserMsgs?.length ? lastUserMsgs[0].createdAt.toLocaleString() : 'never'}`,
        },
      ],
    },

    // -- Response Guidelines / Roleplay Rules --
    {
      heading: isRoleplay ? 'Roleplay Rules' : 'Response Guidelines',
      body: isRoleplay
        ? [
            {
              list: [
